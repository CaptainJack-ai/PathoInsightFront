...
const SimilarRetrievalShowcase = ({ stage, caseData }) => {
  ...
  const [focusIndex, setFocusIndex] = useState(Math.floor((PATCH_DISPLAY_COUNT - 1) / 2));
  ...
  useEffect(() => {
    setFocusIndex(Math.floor((displayRows.length - 1) / 2));
      ...
      <div data-retrieval-scroll="true" ref={retrievalRef} onWheel={onCarouselWheel} className="relative z-10 h-full overflow-visible px-5 pb-6 pt-14 md:px-8">
        <div className="absolute left-1/2 top-1/2 h-[110%] w-full max-w-[1220px] -translate-x-1/2 -translate-y-1/2">
          {displayRows.map((item, idx) => {
            ...
            const scale = absOffset === 0 ? 1 : absOffset === 1 ? 0.72 : absOffset === 2 ? 0.48 : 0.32;
            const opacity = absOffset === 0 ? 1 : absOffset === 1 ? 0.46 : absOffset === 2 ? 0.2 : 0.1;
