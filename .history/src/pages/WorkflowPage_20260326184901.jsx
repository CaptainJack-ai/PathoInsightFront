...
const ImagePreviewModal = ({ imageSrc, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/88 p-4" onClick={onClose}>
      <img
        src={imageSrc}
        alt="Zoomed pathology"
        className="max-h-[90vh] max-w-[90vw] rounded-xl border border-white/30 object-contain"
        onClick={(event) => event.stopPropagation()}
      />
      <button
        type="button"
        onClick={onClose}
        className="absolute right-4 top-4 rounded-full border border-white/45 bg-black/65 px-3 py-1 text-xs uppercase tracking-wider text-white"
      >
        Close
      </button>
    </div>
...
const SimilarRetrievalShowcase = ({ stage, caseData }) => {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/25 bg-black text-blue-100 shadow-[0_20px_80px_rgba(0,0,0,0.32)]">
      ...
      <div data-retrieval-scroll="true" ref={retrievalRef} className="relative z-10 h-full overflow-x-auto overflow-y-auto px-5 pb-6 pt-14 md:px-8">
        <div className="h-full min-w-[1500px] py-2">
            ...
            {displayRows.map((item, idx) => (
              <div key={item.id} className="retrieval-row flex items-center gap-4">
                <div className="w-[22rem]" style={{ transform: `scale(${sizeScales[idx] || 0.84})`, transformOrigin: "left center" }}>
                  <div className="rounded-xl border border-yellow-300/45 bg-black/70 p-2 backdrop-blur-sm">
                    <p className="text-[10px] uppercase tracking-wider text-yellow-300">Top Attention Patch {idx + 1}</p>
                    <button type="button" className="mt-2 block w-20" onClick={() => setPreviewImage(item.sourcePatchImage)}>
                      <img
                        src={item.sourcePatchImage}
                        alt={`Source attention patch ${idx + 1}`}
                        className="aspect-square w-full rounded-md border border-white/20 object-cover transition hover:opacity-90"
                      />
                    </button>
                    <p className="mt-1 truncate text-[10px] text-blue-100/65">{item.sourcePatchPath}</p>
                  </div>
                </div>
                ...
                <div className="w-[18rem]">
                  <div className="rounded-xl border border-white/35 bg-black/65 p-2">
                    <button type="button" className="block w-full" onClick={() => setPreviewImage(item.similarWsiImage)}>
                      <img
                        src={item.similarWsiImage}
                        alt={`Similar pathology WSI ${idx + 1}`}
                        className="h-16 w-full rounded-md object-cover transition hover:opacity-90"
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="min-h-16 w-[34rem] rounded-xl border border-white/30 bg-black/65 p-3">
                  ...
                  <p className="mt-1 line-clamp-2 text-[11px] text-blue-100/75">{item.similarReportText}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="grid w-full max-w-3xl grid-cols-1 gap-3 md:grid-cols-3">
                {patchHighlights.slice(0, 3).map((img, idx) => (
                  <div key={`top-${img}`} className="highlight-patch-card rounded-xl border border-white/30 bg-black/65 p-1.5 shadow-[0_8px_30px_rgba(0,0,0,0.3)]">
                    <button type="button" className="block w-full" onClick={() => setPreviewImage(img)}>
                      <img src={img} alt={`Top attention patch ${idx + 1}`} className="aspect-square w-full rounded-md object-cover transition hover:opacity-90" />
                    </button>
                  </div>
                ))}
              </div>
              ...
              <div className="grid w-full max-w-[38rem] grid-cols-1 gap-3 md:grid-cols-2">
                {patchHighlights.slice(3, 5).map((img, idx) => (
                  <div key={`bottom-${img}`} className="highlight-patch-card rounded-xl border border-white/30 bg-black/65 p-1.5 shadow-[0_8px_30px_rgba(0,0,0,0.3)]">
                    <button type="button" className="block w-full" onClick={() => setPreviewImage(img)}>
                      <img src={img} alt={`Bottom attention patch ${idx + 4}`} className="aspect-square w-full rounded-md object-cover transition hover:opacity-90" />
                    </button>
                  </div>
                ))}
              </div>
...
function WorkflowPage() {
  ...
  const handleWheel = (event) => {
    ...
    if (activeStage?.id === "retrieving_similar") {
      const retrievalScroller = event.currentTarget.querySelector('[data-retrieval-scroll="true"]');
      if (retrievalScroller) {
        retrievalScroller.scrollLeft += event.deltaY;
      }
      return;
    }
