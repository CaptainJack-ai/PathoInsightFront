...
function AiDiagramPage() {
  const nodes = useMemo(
    () => [
      ...
      { id: "outA", data: { label: "最终报告 PDF" }, position: { x: 3720, y: 320 }, targetPosition: Position.Left, style: outputStyle },
    ],
    []
  );
  ...
  const edges = useMemo(
    () => [
      ...
      { id: "e-critic-out", source: "critic", target: "outA", animated: true, label: "达阈值输出", thread: "main" },
    ].map((edge) => ({
      ...edge,
      type: "smoothstep",
      markerEnd: {
