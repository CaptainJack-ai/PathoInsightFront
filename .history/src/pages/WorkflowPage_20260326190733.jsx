  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/25 bg-black text-blue-100 shadow-[0_20px_80px_rgba(0,0,0,0.32)]">
      ...
      <div data-retrieval-scroll="true" ref={retrievalRef} onWheel={onCarouselWheel} className="relative z-10 h-full overflow-hidden px-5 pb-6 pt-14 md:px-8">
        <div className="absolute left-1/2 top-1/2 h-[88%] w-full max-w-[1220px] -translate-x-1/2 -translate-y-1/2">
          {displayRows.map((item, idx) => {
            const offset = idx - focusIndex;
            const absOffset = Math.abs(offset);
            const scale = absOffset === 0 ? 1 : absOffset === 1 ? 0.86 : 0.74;
            const opacity = absOffset === 0 ? 1 : absOffset === 1 ? 0.58 : 0.3;
            const blur = absOffset === 0 ? 0 : absOffset === 1 ? 2 : 5;
            const translateY = offset * 190;

            return (
              <article
                key={item.id}
                className="retrieval-card absolute left-1/2 top-1/2 w-full max-w-[1180px] transition-all duration-300 ease-out"
                style={{
              ...
              >
                <div className="grid grid-cols-[220px_34px_220px_34px_minmax(0,1fr)] items-center gap-3 rounded-2xl border border-white/20 bg-black/68 p-3 backdrop-blur-sm">
                  <div>
                  ...
                  <div className="min-h-[12rem] rounded-xl border border-white/25 bg-black/60 p-3">
