...
const SimilarRetrievalShowcase = ({ stage, caseData }) => {
  ...
  useGSAP(
    () => {
      ...
      const bridgePatches = retrievalRef.current?.querySelectorAll(".bridge-patch");
      if (!rows?.length) return;

      gsap.set(rows, { autoAlpha: 0, x: 18 });

      if (bridgePatches?.length) {
        gsap.set(bridgePatches, { autoAlpha: 1 });
        gsap
          .timeline()
          .to(bridgePatches, {
            left: (idx) => `${4 + idx * 0.3}%`,
            top: (idx) => `${13 + idx * 16}%`,
            width: (idx) => `${170 + (sizeScales[idx] || 0.84) * 70}px`,
            height: (idx) => `${96 + (sizeScales[idx] || 0.84) * 40}px`,
            duration: 0.65,
            stagger: 0.06,
            ease: "power3.inOut",
          })
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/25 bg-black text-blue-100 shadow-[0_20px_80px_rgba(0,0,0,0.32)]">
      ...
      <div className="pointer-events-none absolute inset-0 z-[11] hidden md:block">
        {patchHighlights.slice(0, 5).map((img, idx) => (
          <div
            key={`bridge-${img}`}
            className="bridge-patch absolute overflow-hidden rounded-lg border border-yellow-300/45 shadow-[0_8px_24px_rgba(0,0,0,0.35)]"
            style={{
              left: idx < 3 ? `${34 + idx * 12}%` : `${40 + (idx - 3) * 12}%`,
              top: idx < 3 ? "34%" : "54%",
              width: "180px",
              height: "104px",
            }}
          >
            <img src={img} alt={`Bridge patch ${idx + 1}`} className="h-full w-full object-cover" />
          </div>
        ))}
      </div>

      <div data-retrieval-scroll="true" ref={retrievalRef} className="relative z-10 h-full overflow-x-auto overflow-y-hidden px-5 pb-6 pt-14 md:px-8">
