  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/25 bg-black text-blue-100 shadow-[0_20px_80px_rgba(0,0,0,0.32)]">
      <div className="relative h-[64%] overflow-hidden">
        <img src={stageImage} alt={stage.title} className="h-full w-full object-cover" />
...
function WorkflowPage() {
  ...
  const isClassifyingStage = activeStage.id === "classifying";
  const uploadedPreviewImage =
    selectedCase?.stageImages?.uploaded || STAGES[0].fallbackImage;
        ...
        <div className="flex h-[calc(100%-5rem)] flex-col gap-6 md:flex-row md:gap-8">
          <aside className="flex h-full md:w-[30%] md:min-w-[320px] md:max-w-[480px] md:items-center">
            <div className="w-full rounded-2xl border border-black/15 bg-white/90 p-6 shadow-[0_12px_50px_rgba(1,1,1,0.08)] md:p-7">
              ...
              {selectedCase && isClassifyingStage && (
                <div className="mt-4 overflow-hidden rounded-xl border border-black/15 bg-black/5 p-2">
                  <div className="relative h-44 overflow-hidden rounded-lg md:h-52">
                    <img
                      src={uploadedPreviewImage}
                      alt="uploaded wsi preview"
                      className="h-full w-full object-cover"
                    />
                    <div className="absolute inset-0 bg-black/20" />
                    <div className="absolute left-1/2 top-1/2 w-[88%] max-w-[260px] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-yellow-300/60 bg-black/78 px-3 py-3 text-center backdrop-blur-sm">
                      <p className="text-[10px] uppercase tracking-wider text-yellow-300">
                        Classification
                      </p>
                      <p className="mt-2 font-zentry text-2xl uppercase leading-none text-blue-100">
                        CLASS_LABEL_PLACEHOLDER
                      </p>
                    </div>
                  </div>
                </div>
