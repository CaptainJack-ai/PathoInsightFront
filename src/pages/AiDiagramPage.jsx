...
function AiDiagramPage() {
  ...
  const nodes = useMemo(
    () => [
      ...
      { id: "doctorDiagnosis", data: { label: "真实病例医生诊断结果\n(人工复核+诊断录入)" }, position: { x: 4050, y: 390 }, sourcePosition: Position.Right, targetPosition: Position.Left, style: infoStyle },
    ],
    []
  );
  ...
  const edges = useMemo(
    () => [
      ...
      { id: "e-out-a-doctor", source: "outA", target: "doctorDiagnosis", label: "汇总", thread: "main" },
      { id: "e-out-b-doctor", source: "outB", target: "doctorDiagnosis", label: "汇总", thread: "alt" },
      { id: "e-doctor-reports", source: "doctorDiagnosis", target: "reportsJson", label: "反馈回路，丰富参考库", thread: "feedback", dashed: true },
    ].map((edge) => ({
      ...edge,
      type: "smoothstep",
      markerEnd: {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#0c1a21] text-slate-100">
      ...
      <div className="absolute right-4 top-20 z-20 md:right-6 md:top-24">
        <button
          type="button"
          onClick={() => setShowSystemArchitecture(true)}
          className="rounded-full border border-cyan-300/30 bg-slate-950/75 px-4 py-2 text-sm font-semibold text-cyan-100 shadow-[0_10px_30px_rgba(2,6,23,0.35)] backdrop-blur-md transition hover:border-cyan-200/60 hover:bg-slate-900/90 hover:text-white"
        >
          面向开发者的系统架构图
        </button>
      </div>
      ...
      {showSystemArchitecture && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-white/95 px-4 py-6 text-slate-900 backdrop-blur-sm">
          <div className="relative flex max-h-[92vh] w-full max-w-[min(96vw,1600px)] flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.18)]">
            <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 md:px-6">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">面向开发者的系统架构图</h2>
                <p className="mt-1 text-sm text-slate-500">来源：public/img/AIdrawio.svg</p>
              </div>
              <button
                type="button"
                onClick={() => setShowSystemArchitecture(false)}
                className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:bg-slate-100 hover:text-slate-900"
              >
                关闭
              </button>
            </div>

            <div className="flex min-h-0 flex-1 items-center justify-center bg-slate-50 p-3 md:p-5">
              <img
                src="/img/AIdrawio.svg"
                alt="面向开发者的系统架构图"
                className="max-h-[calc(92vh-8.5rem)] w-full object-contain"
              />
            </div>
          </div>
        </div>
