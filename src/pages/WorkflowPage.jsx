...
const STAGES = [
  ...
  { id: "generating_report", title: "报告生成", subtitle: "生成诊断报告与推理报告", badge: "步骤 05", fallbackImage: "/img/patho-lab-2.webp" },
];
...
const ReportPdfShowcase = ({ stage, caseData }) => {
  ...
  const reasoningReportPdf = caseData?.reasoningReportPdf;
  const hasReasoningReport = reasoningReportPdf != null;
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-black/15 bg-blue-50 text-black shadow-[0_20px_80px_rgba(0,0,0,0.18)]">
      ...
      <div className={`relative z-10 flex h-full flex-col px-3 pb-3 pt-11 sm:px-4 sm:pb-4 sm:pt-12 md:px-5 md:pb-5 ${hasReasoningReport ? "" : ""}`}>
        {/* ===== 诊断报告 ===== */}
        <div className={`flex flex-col ${hasReasoningReport ? "mb-3 h-[calc(50%-6px)]" : "h-full"}`}>
          <div className="mb-1.5 flex items-center justify-between gap-2">
            <p className="font-general text-xs uppercase tracking-wider text-black/65">诊断报告 PDF</p>
            <a
              href={reportPdf}
              download
              className="rounded-full border border-black/25 bg-white/85 px-3 py-1 font-general text-[9px] uppercase tracking-wider text-black transition hover:bg-black hover:text-white sm:px-4 sm:text-[10px]"
            >
              下载 PDF
            </a>
          </div>
          <div className="min-h-0 flex-1 overflow-auto rounded-xl border border-black/15 bg-white p-1 shadow-[0_8px_30px_rgba(0,0,0,0.08)]">
            <iframe
        ...
        {/* ===== 推理报告 ===== */}
        {hasReasoningReport && (
          <div className="flex h-[calc(50%-6px)] flex-col">
            <div className="mb-1.5 flex items-center justify-between gap-2">
              <p className="font-general text-xs uppercase tracking-wider text-black/65">推理报告 PDF</p>
              <a
                href={reasoningReportPdf}
                download
                className="rounded-full border border-black/25 bg-white/85 px-3 py-1 font-general text-[9px] uppercase tracking-wider text-black transition hover:bg-black hover:text-white sm:px-4 sm:text-[10px]"
              >
                下载 PDF
            ...
            <div className="min-h-0 flex-1 overflow-auto rounded-xl border border-black/15 bg-white p-1 shadow-[0_8px_30px_rgba(0,0,0,0.08)]">
              <iframe
                title={`${caseData.name} 推理报告`}
                src={`${reasoningReportPdf}#view=FitH`}
                className="h-full w-full rounded-md bg-white"
              />
            </div>
          </div>
